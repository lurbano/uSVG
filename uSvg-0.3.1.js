...
class uPolygon{
  ...
  constructor({
                pts=[new uPoint(), new uPoint(0,1), new uPoint(1,0)],
                rotate = 0,
                rotateAxis = new uPoint(),
                translate = new uPoint()
              }={}){
    this.pts = pts;
    this.translate = translate;
    this.rotate = rotate;
    this.rotation(rotate, rotateAxis);
    this.translation(translate);
    this.setVertices();
  }

  translation(p=new uPoint()){
    for (let i=0; i<this.pts.length; i++){
      this.pts[i].move(p);
    }
  }

  rotation(angle=0, axis=new uPoint()){
    for (let i=0; i<this.pts.length; i++){
      this.pts[i] = this.pts[i].rotateAxis(angle, axis);
    }
  }
  ...
  setLineSegments(){
    ...
    for (let i=0; i<this.pts.length-1; i++){
      this.lineSegments.push(new uLineSegment(this.pts[i], this.pts[i+1]));
    }
  }
  ...
  drawAngleLabel({ n=0, angleLabel='use_angle', angleLabelRounding=0, arc_r=3, style={}
                 } = {}){
   if (this.vertices[n].label !== undefined){ this.vertices[n].label.remove()};
   //add new arc
   this.vertices[n].label = svg.addAngleLabel({
     arc_r: arc_r,
     vertex: this.vertices[n],
     angle_label: angleLabel,
     angle_label_rounding: angleLabelRounding,
     style: style
   });
...
class uRegularPolygon{
  ...
  constructor({nsides=6, sideLength=1, translate=new uPoint(), rotate=0} = {}){
    this.nsides = nsides;
